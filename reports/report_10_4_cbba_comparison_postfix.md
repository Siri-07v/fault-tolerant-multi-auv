...
### Interpretive Commentary on Statistical Significance
The following interpretations clarify the practical context behind the statistically significant outcomes highlighted in Table 10.4.2:
- **Task Completion Rate (Mass Fault Stress):** Under extreme fault conditions (Mass Fault Stress), the classic CBBA baseline achieves a statistically significant advantage in completion rate. This is because the proposed system adopts a highly conservative task reallocation policy upon detecting faults to prevent cascading failures and energy waste, whereas the classic CBBA baseline lacks fault-awareness and continues aggressive, high-risk task execution.
- **Deadlock Frequency (Nominal Baseline):** Although the difference is statistically significant, this is primarily a logging artifact. The classic CBBA baseline does not possess any deadlock detection or logging mechanisms; consequently, it records a deadlock frequency of exactly 0.00, despite suffering from unlogged vehicle stalls. The proposed system actively monitors, logs, and resolves deadlocks, which are accurately reflected in its metrics.
- **Congestion Drop Rate (All Scenarios):** The proposed system achieves a statistically significant reduction in congestion drop rate across all tested scenarios. This confirms that the Auction+EDMC coordination protocol significantly optimizes message exchange and reduces network overhead compared to the baseline CBBA.

### Exclusion of Stalled AMV-Timesteps from Significance Testing
The **Stalled AMV-Timesteps** metric was explicitly excluded from formal Wilcoxon signed-rank testing due to a combination of a zero-variance data column and a structural definitional mismatch between the two systems:
1. **Mathematical Invalidity of the p-value (Zero Variance):** In all 15 simulation seeds across all scenarios, the proposed system recorded a flat stalled timestep count of exactly 0.00 ± 0.00. Given this zero-variance column, the resulting Wilcoxon p-value is **mathematically degenerate and statistically invalid**. The signed-rank test requires stochastically varying paired differences; with one treatment acting as a constant zero, the test violates fundamental distributional assumptions and merely assesses whether the baseline values are non-zero rather than performing a valid comparative hypothesis test.
2. **Definitional Mismatch:** The metric is defined and tracked differently across the two allocators. In the Proposed system, "stalled timesteps" are strictly accumulated when an AMV is actively stuck in the `FSM_DEADLOCK` state. Because the EDMC protocol detects and resolves deadlocks almost instantaneously (typically within a single timestep), vehicles exit the deadlock state immediately, resulting in 0 stalled timesteps. In contrast, the classic CBBA baseline does not have an FSM-level deadlock state; its stalled timesteps capture a broader operational condition where vehicles are physically blocked or unable to make progress.

Although excluded from statistical significance testing, the raw descriptive statistics remain in the KPI table (Table 10.4) for completeness. Qualitatively, the proposed system demonstrates a major reduction in operational stalling (0.00 timesteps) compared to the baseline CBBA (which accumulates between 116.27 and 247.80 stalled timesteps due to its lack of deadlock recovery), representing a clear operational improvement that is best understood through qualitative inspection rather than a paired statistical test.

### Mechanistic Explanation of the Nominal-vs-Comm-Blackout Comparison
Across the simulation runs, the Task Completion Rate for classic CBBA under Nominal Baseline and Communication Blackout Stress scenarios shows identical average results ($92.89\% \pm 5.89\%$). Rather than a lack of dynamic reallocation capability, this is explained by the following physical mechanism of the simulation:
- **High Lock and Completion Rate at t=150:** In classic CBBA, task assignments are deconflicted via consensus rounds. By the time the communication blackout begins at $t=150$, the vast majority of tasks are already resolved. Specifically, for a representative run (Seed 42), out of 15 tasks, **9 tasks (60%)** are already completed and **4 tasks (26.7%)** are active and locked by vehicles in the Reaching or Serving states.
- **Dynamic Task Lock:** Reaching and Serving vehicles lock their assigned tasks at a massive bid value ($10^9$) to prevent mid-execution outbidding. Consequently, only **2 tasks (13.3%)** remain unassigned and contestable at $t=150$. Because so few tasks are open for bidding during the blackout window ($t \in [150, 300]$), the packet loss has virtually no operational impact on CBBA's task allocation decisions, leading to identical task completion statistics.
