...
def trace_none_surfacing():
    ...
    def mock_update_dvl_drift(self, distance_moved):
        if config.DVL_RESET_ON_SURFACE and self.depth < 16.0:
            print(f"  [SURFACE EVENT] t=??: AMV{self.amv_id} at surface (depth={self.depth:.1f}m, state={self.fsm_state}, dive_phase={self.dive_phase})")
    ...
    print("SURFACING LOGS FOR STRESS=NONE:")
    count = 0
    for line in lines:
        if "SURFACE" in line:
            print(line)
            count += 1
    print(f"Total surface logs: {count}")

if __name__ == "__main__":
    trace_none_surfacing()
